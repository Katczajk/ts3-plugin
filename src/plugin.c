...
/* first we take #define to use plugin name and alternate Name */
/* i think better read for code - declare in head of File plugin.c */
const char* ts3plugin_name() {
#ifdef _WIN32
	/* TeamSpeak expects UTF-8 encoded characters. Following demonstrates a possibility how to convert UTF-16 wchar_t into UTF-8. */
	static char* result = NULL;  /* Static variable so it's allocated only once */
	if(!result) {
		const wchar_t* name = PLUGIN_NAME;
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = PLUGIN_NAME_ALT;  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return PLUGIN_NAME_ALT;
#endif
}
...
const char* ts3plugin_author() {
    ...
    return "TS3 Plugin by IT-Pilz (C)2020";
}
